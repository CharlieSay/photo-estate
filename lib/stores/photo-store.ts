...
import { type PhotoJob, type ProcessingSpeed, type PhotoStatus } from '@/types/project'
...
const calculatePriority = (speed: ProcessingSpeed, timestamp: number): number => {
  ...
  persist(
    (set, get) => ({
      ...
      addJob: (job) => {
        ...
        set((state) => ({
          queue: [...state.queue, {
            ...
            priority,
            status: job.speed === 'expedited' ? 'processing' : 'queued'
          }]
        }))
      ...
      updateJob: (id, updates) => set((state) => ({
        ...
        activeJobs: updates.status === 'completed' || updates.status === 'failed' 
          ? state.activeJobs - 1 
          : state.activeJobs
      })),
      ...
      removeJob: (id) => set((state) => ({
        ...
        activeJobs: state.queue.find(job => job.id === id)?.status === 'processing' 
          ? state.activeJobs - 1 
          : state.activeJobs
      })),
      ...
      clearCompletedJobs: (projectId) => set((state) => ({
        queue: state.queue.filter(job => 
          job.projectId !== projectId || job.status !== 'completed'
        )
      })),
      ...
      processNextJob: async () => {
        ...
        try {
          ...
          for (const enhancement of nextJob.enhancements) {
            await new Promise(resolve => setTimeout(resolve, processingTime / (2 * nextJob.enhancements.length)))
